...
curl -X POST http://localhost:3409/api/account/execute \
  -H "Content-Type: application/json" \
  -d '{
    "tabId": "wechat-1754806014246",
    "script": "(async function() { try { console.log(\"🔥 开始测试定时发布设置...\"); const wujieApp = document.querySelector(\"wujie-app\"); if (!wujieApp || !wujieApp.shadowRoot) { throw new Error(\"未找到Shadow DOM\"); } const shadowDoc = wujieApp.shadowRoot; console.log(\"✅ 找到Shadow DOM\"); const timePickerInput = shadowDoc.querySelector(\".weui-desktop-picker__date input[placeholder*=\\\"请选择发表时间\\\"]\"); console.log(\"时间选择器输入框:\", timePickerInput); if (!timePickerInput) { const allInputs = shadowDoc.querySelectorAll(\"input\"); console.log(\"所有输入框:\", Array.from(allInputs).map(inp => ({placeholder: inp.placeholder, type: inp.type, class: inp.className}))); throw new Error(\"未找到时间选择器输入框\"); } timePickerInput.click(); console.log(\"✅ 已点击时间选择器\"); await new Promise(resolve => setTimeout(resolve, 1500)); const dayLinks = shadowDoc.querySelectorAll(\"a[href=\\\"javascript:;\\\"]\"); console.log(\"找到日期链接数量:\", dayLinks.length); const availableDays = Array.from(dayLinks).filter(link => !link.classList.contains(\"weui-desktop-picker__faded\") && !link.classList.contains(\"weui-desktop-picker__disabled\")); console.log(\"可用日期:\", availableDays.map(link => link.textContent.trim())); if (availableDays.length > 0) { const targetDay = availableDays.find(link => parseInt(link.textContent.trim()) > 10) || availableDays[0]; targetDay.click(); console.log(\"✅ 已选择日期:\", targetDay.textContent.trim()); await new Promise(resolve => setTimeout(resolve, 500)); } const timeInput = shadowDoc.querySelector(\".weui-desktop-picker__time input[placeholder*=\\\"请选择时间\\\"]\"); console.log(\"时间输入框:\", timeInput); if (timeInput) { timeInput.click(); await new Promise(resolve => setTimeout(resolve, 500)); const hourList = shadowDoc.querySelector(\".weui-desktop-picker__time__hour\"); const minuteList = shadowDoc.querySelector(\".weui-desktop-picker__time__minute\"); console.log(\"小时列表:\", hourList); console.log(\"分钟列表:\", minuteList); if (hourList) { const hourItems = hourList.querySelectorAll(\"li\"); console.log(\"小时选项数量:\", hourItems.length); if (hourItems[14]) { hourItems[14].click(); console.log(\"✅ 已设置小时: 14\"); } } if (minuteList) { const minuteItems = minuteList.querySelectorAll(\"li\"); console.log(\"分钟选项数量:\", minuteItems.length); if (minuteItems[30]) { minuteItems[30].click(); console.log(\"✅ 已设置分钟: 30\"); } } } console.log(\"✅ 定时发布设置测试完成\"); return { success: true, message: \"定时发布设置成功\" }; } catch (e) { console.error(\"❌ 定时发布设置失败:\", e); return { success: false, error: e.message }; } })()"
  }'
